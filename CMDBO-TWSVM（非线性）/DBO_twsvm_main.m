...
for i=1:num
   ...
   t1 = clock
    ...
    t1 = clock
    [fMin,bestX_CMDBO,CMDBO_curve]=CMDBO(data_train,Y_train,SearchAgents_no,Max_iteration,lb,ub,dim,fobj,ker);
    t2 = clock
    disp(['CMDBO_curve运行时间: ',num2str(etime(t1,t2))]);
    time_CMDBO = num2str(etime(t2,t1))
    figure(1)
    semilogy(PSO_curve,'Color','g');
    title('Pima-Indian nonlinear experiment convergence curves');
    xlabel('Iteration');
    ...
    best_c1 = bestX_CMDBO(1);
    best_c2 = bestX_CMDBO(2);
    best_s = bestX_CMDBO(3);
